...
class BrowserSession:
    ...
    def interactive_login(self, login_url: str, validation_text: str) -> None:
        self.logger.info("Abrindo navegador persistente para login manual no contexto '%s'.", self.browser_context.id)
        with self.open_page(force_headed=True) as page:
            page.goto(login_url, wait_until="domcontentloaded")
            input(
                "Conclua o login manual no navegador aberto e pressione ENTER aqui quando a tela correta estiver visivel."
            )

            if not self.is_authenticated(page, validation_text):
                raise AuthenticationRequiredError(
                    f"A tela esperada '{validation_text}' nao ficou acessivel no contexto '{self.browser_context.id}'."
                )
