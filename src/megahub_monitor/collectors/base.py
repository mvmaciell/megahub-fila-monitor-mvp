...
class BaseQueueCollector:
    ...
    def collect(self, page: Page) -> list[Ticket]:
        if not self.source.first_page_only:
            self.logger.warning("A leitura alem da primeira pagina ainda nao esta implementada para '%s'.", self.source.id)
        ...
        self.logger.info("Captura concluida para '%s' com %s chamado(s) visivel(is).", self.source.id, len(tickets))
        return tickets
    ...
    def _ensure_page_is_ready(self, page: Page) -> None:
        try:
            page.locator(f"text={self.page_title}").first.wait_for(timeout=self.settings.playwright_timeout_ms)
        except Exception as exc:
            raise AuthenticationRequiredError(
                f"Nao foi possivel acessar a tela '{self.page_title}' para a fonte '{self.source.id}'."
            ) from exc
    ...
    def _ensure_checkbox(self, page: Page, label_text: str, expected: bool) -> None:
        ...
        if result.get("found"):
            self.logger.info("Fonte '%s': filtro '%s' ajustado para %s.", self.source.id, label_text, expected)
    ...
    def _wait_for_grid(self, page: Page) -> None:
        try:
            ...
            raise CollectionError(f"A grade da fonte '{self.source.id}' nao ficou disponivel para leitura.") from exc
    ...
    def _build_tickets(self, extracted: dict) -> list[Ticket]:
        ...
        if not headers:
            raise CollectionError(f"Nao foi possivel identificar a tabela principal da fonte '{self.source.id}'.")
